...
This project provides seamless integration of the [Telegram Login Widget](https://core.telegram.org/widgets/login) with Spring Security. It includes a Spring Boot starter for quick setup and a dedicated Spring Security configurer for manual configuration.
...
- **Extensible Architecture**: Custom validators and converters can be easily plugged in.
...
3. `TelegramAuthenticationConverter` extracts this data and creates a `TelegramAuthenticationToken`.
4. `TelegramAuthenticationProvider` validates the token:
    - `HashValidator` verifies the HMAC-SHA256 signature using your bot token.
    - `AuthDateExpirationValidator` ensures the data is fresh.
5. If valid, the user is authenticated with the `TELEGRAM` authority factor.

## Samples

Check the [samples/mvc-sample](samples/mvc-sample) directory for a complete working example of a Spring Boot MVC application with Telegram Login.
